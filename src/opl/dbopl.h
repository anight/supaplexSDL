/*
 * DBOPL - the DOSBox OPL emulator, as vendored into picopop.
 *
 * Taken from dosbox-x (src/hardware/dbopl.{cpp,h}), GPLv2-or-later, and trimmed
 * to the emulator core: DOSBox's DBOPL::Handler - the glue to its mixer, its
 * port-I/O decoding and its save-state serialisation - has been removed, which
 * is what let the dosbox.h and adlib.h dependencies go with it. Only the
 * typedefs remain, in dbopl_types.h.
 *
 * Why this and not Nuked, which the game shipped with: measured on the title
 * theme, DBOPL costs 938 instructions per output sample against Nuked's 5583
 * after optimisation - about 6x cheaper, and the difference between ~21M and
 * ~123M instructions per second on a 128 MHz core. The two are level-matched to
 * 0.06 dB with a 0.9995 loudness-envelope correlation; DBOPL is about 1.3 dB
 * brighter above 6 kHz. Nuked is the more exact chip model; this one fits.
 *
 * opl3.c compiles, but only the host tests build it
 * (`make -C tools/tests music OPL=nuked`); the firmware has no switch for it.
 */
/*
...
#else
	/* const: the wave table is read-only, and with
	 * PICOPOP_DBOPL_CONST_TABLES it lives in flash. Never written through. */
	const int16_t* waveBase = NULL;
	uint32_t waveMask = 0;
...
// Builds the shared wave and multiplication tables. DOSBox calls this from
// DBOPL::Handler::Init, which is one of the pieces removed here, so the caller
// has to. Miss it and the chip runs but plays silence.
...
/* DOSBox's DBOPL::Handler removed - see the note at the top. */
